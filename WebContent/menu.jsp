...
<%@ page import="com.ftc.vo.SysMenuVO" %>
					...
					if("1".equals(String.valueOf(assessSysMenuVO.getMenuLevel().intValue()))){			%>			 
						...
						<%
							if(assessSysMenuVO.getMenuUrl()==null || "".equals(assessSysMenuVO.getMenuUrl()) || "#".equals(assessSysMenuVO.getMenuUrl())){
								...
								%>							  				 						 						 
								<a href="<%=context%><%=assessSysMenuVO.getMenuUrl()%>&menuNme=<%=assessSysMenuVO.getMenuNme() %>&menuId=<%=assessSysMenuVO.getMenuId() %>"><%=URLDecoder.decode(assessSysMenuVO.getMenuNme(),"UTF-8")   %></a>
							...
							%>
											<%
												if(assessSysMenuVO2.getMenuUrl()==null || "".equals(assessSysMenuVO2.getMenuUrl()) || "#".equals(assessSysMenuVO2.getMenuUrl())){
													...
													%>													
														<li>														
															<a href="<%=context%><%=assessSysMenuVO2.getMenuUrl() %>&menuNme=<%=assessSysMenuVO2.getMenuNme() %>&menuId=<%=assessSysMenuVO2.getMenuId() %>"><%=assessSysMenuVO2.getMenuNme() %></a>
